    ...
    <div class="user-main">
        <div class="main-wrap">
            <div class="fav-main">
                ...
                <!-- tag -->
                <div class="vv-tag mt10 clearfix">
                    <ul>
                        <li<c:if test="${param.type eq null}"> class='curr'</c:if>><a href="/user/favorite.html">上架商品</a></li>
                        <li<c:if test="${param.type eq 'down'}"> class='curr'</c:if>><a href="/user/favorite.html?type=down">下架商品</a></li>
                    </ul>
                    <c:if test="${param.type == null}">
                    <div class="order-search fr">
                        <a id="searchOrder">搜索</a>
                        <input type="text" id="Q" value="${param.q}" placeholder="货号" />
                    </div>
                    </c:if>
                                                ...
                                                <c:when test="${item.is_pre_sell==1 || item.art_no=='' || item.support_cart != 1}">
                                                    <div class="ctrl one clearfix">
                                                        <a href="javascript:;" class="j-upload" data-id="${item.tid}" data-vid="${item.id}"
                                                              data-price="${item.discount_price}" vda="yjsc|itemcard|${item.id}|0|0|${status.count}"><span>一键上传</span>
                                                        </a>
                                                    </div>
                                                </c:when>
                                                <c:otherwise>
                                                    <div class="ctrl clearfix">
                                                        ...
                                                        <span class="j-upload upload" data-id="${item.tid}" data-vid="${item.id}"
                                                              data-price="${item.discount_price}" vda="yjsc|itemcard|${item.id}|0|0|${status.count}">一键上传
                                                        </span>
                                                    </div>
                                                </c:otherwise>
                                            </c:choose>
                                ...
                                <div class="desc">
                                    <div class="info clearfix">
                                        <span class="fr up_time">
                                            <fmt:formatDate  value="${item.ctime}" pattern="M月dd日"/> 收藏
                                        </span>
                                        <c:choose>
                                            <c:when test="${item.status==1}">
                                                <div class="price">
