...
"use strict";
var path = window._WEB_Cfg.jsLibPath + 'jquery.layer/', //组件存放目录，为空表示自动获取(不用填写host，相对站点的根目录即可)。
$, win, ready = {
    getPath: function(){
...
var layer = {
    ...
    stock: function(content, follow, cb){
        var stockLayer = layer.tips(content, follow, {tips: [2, '#FEFAEC'], skin:'stock-less',time:1000})
        follow.bind('mouseleave', function(){
            layer.close(stockLayer)
        })
    },

    vtips: function(content, follow){
        var vtipsLayer = layer.tips(content, follow, {tips: [3, '#FEFAEC'], skin:'vtips',time:100000})
        follow.bind('mouseleave', function(){
            // layer.close(vtipsLayer)
        })
    }
};
        ...
        '<div class="'+ doms[0] +' '+ (doms.anim[config.shift]||'') + (' layui-layer-'+ready.type[config.type]) + (((config.type == 0 || config.type == 2) && !config.shade) ? ' layui-layer-border' : '') + ' ' + (config.skin||'') +'" id="'+ doms[0] + times +'" type="'+ ready.type[config.type] +'" times="'+ times +'" showtime="'+ config.time +'" conType="'+ (conType ? 'object' : 'string') +'" style="z-index: '+ zIndex +'; width:'+ config.area[0] + ';height:' + config.area[1] + (config.fix ? '' : ';position:absolute;') +'">'
            ...
            + (config.btn ? function(){
                ...
                for(var i = 0, len = config.btn.length; i < len; i++){
                    var _target = config.target ? config.target[i] : '';
                    if(_target=='_blank'){
                        button += '<a class="'+ doms[6] +''+ i +'" target="_blank" class="link" href="'+config.src[i]+'">'+ config.btn[i] +'</a>'
                    }else{
...
Class.pt.creat = function(){
    ...
    switch(config.type){
        ...
        case 4:
            ...
            config.content = config.content[0] + '<i class="layui-layer-TipsG"></i><i class="layui-layer-TipsS"></i>';
            config.title = false;
            config.shade = false;
            config.fix = false;
...
Class.pt.callback = function(){
    ...
    //按钮
    layero.find('.'+ doms[6]).children('a:not(.link)').on('click', function(){
